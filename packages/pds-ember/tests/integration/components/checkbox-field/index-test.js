...
module('Integration | Components.CheckboxField', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.dom(ROOT).exists().hasNoText().hasClass('pds-checkboxField');
  ...
  test('it yields an auto-generated id', async function (assert) {
    ...
    assert.dom(ROOT).hasText(/chk-ember\d+/, 'yields generated id');
  });
  ...
  test('it supports @invalid', async function (assert) {
    ...
    await settled();
    assert.dom(INPUT).hasClass('pds--invalid', 'does not appear "invalid"');
  });
});
