...
module('Integration | Components.Loading.Elapsed', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.dom(ROOT).containsText('Time Elapsed: 00:01');

    // advance 59 seconds
    this.clock.tick(59 * 1000);

    assert.dom(ROOT).containsText('Time Elapsed: 01:00');

    // advance 59 minutes
    this.clock.tick(59 * 60 * 1000);

    assert.dom(ROOT).containsText('Time Elapsed: 01:00:00');
  });
  ...
  test('it renders: @startTime', async function (assert) {
    ...
    assert.dom(ROOT).containsText('Time Elapsed: 30:01');
  });
});
